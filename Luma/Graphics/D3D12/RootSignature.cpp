...
RootSign CreateRootSignature(const GfxDevice& gfxDevice, RootSignDesc desc)
{
    ...
    if (FAILED(gfxDevice.device_->CheckFeatureSupport(D3D12_FEATURE_ROOT_SIGNATURE,
        &featureData, sizeof(featureData))))
    {
        featureData.HighestVersion = D3D_ROOT_SIGNATURE_VERSION_1_0;
    }

    if (desc._type == RootSignDesc::RSType::RENDER)
    {
        ...
        DX_ASSERT(gfxDevice.device_->CreateRootSignature(0,
            signature->GetBufferPointer(), signature->GetBufferSize(),
            IID_PPV_ARGS(&rootSign._rootSignature)));
    }
    else if (desc._type == RootSignDesc::RSType::DEPTHPP)
    {
        D3D12_ROOT_PARAMETER1 rootParameters[1]{};
    ...
    else if (desc._type == RootSignDesc::RSType::SHADER_EFFECTS || desc._type == RootSignDesc::RSType::RTAO)
    {
        ...
        rootParameters[0].Constants.ShaderRegister = 0;
        {
            if (desc._type == RootSignDesc::RSType::SHADER_EFFECTS) rootParameters[0].Constants.Num32BitValues = sizeof(ShaderEffects) / 4;
            else if (desc._type == RootSignDesc::RSType::RTAO) rootParameters[0].Constants.Num32BitValues = sizeof(RTAO) / 4;
        }
