...
void RasterPass::Init(ResourceManager* resourceManager, PipelineCache* pipelineCache)
{
	...
	*/
	...
	// raster pipeline and the shaders
	{
		...
		.pTarget = L"vs_6_7",
		.type = Type::VERTEX };
		ShaderHandle vsHandle = pipelineCache->LoadShader(vsDesc, "RasterPassVertexShader");
		...
		ShaderDesc psDesc{
			...
			.pTarget = L"ps_6_7",
			.type = Type::PIXEL };
		ShaderHandle psHandle = pipelineCache->LoadShader(psDesc, "RasterPassPixelShader");
