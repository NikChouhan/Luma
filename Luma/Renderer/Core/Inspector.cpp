...
void Inspector::CreateImguiHeap(const GfxDevice& gfxDevice)
{
    D3D12_DESCRIPTOR_HEAP_DESC desc = {};
    desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
    desc.NumDescriptors = APP_SRV_HEAP_SIZE;
    desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
    DX_ASSERT(gfxDevice.device_->CreateDescriptorHeap(&desc, IID_PPV_ARGS(_imguiHeap.GetAddressOf())));
    g_ImGuiHeapAllocator->Create(gfxDevice.device_.Get(), _imguiHeap.Get());
}
...
void Inspector::CreateInspector(GfxDevice& gfxDevice, Swapchain& swapchain, FrameSync& frameSync)
{
    ...
    ImGui_ImplWin32_Init(swapchain.hwnd_);

    ImGui_ImplDX12_InitInfo initInfo = {};
    initInfo.Device = gfxDevice.device_.Get();
    initInfo.CommandQueue = gfxDevice.commandQueue_.Get();
